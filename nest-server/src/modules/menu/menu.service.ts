...
export class MenuService {
  ...
  async tree(): Promise<MenuTreeNode[]> {
    const menus = await this.prisma.sysMenu.findMany({ where: { deleted: 0 } })
  ...
  async create(request: MenuRequest, operatorId: number | null): Promise<number> {
  ...
  async update(id: number, request: MenuRequest, operatorId: number | null): Promise<void> {
    validateMenuRequest(request)
